...
Adjacent List로 그래프를 구현하였을 때 아래 그림과 같은 형태로 이해할 수 있으며, Stack은 DFS의 흐름을 알아보기 그렸지만, 실제 구현에서는 쓰이지 않을 수 있다. 
아래의 순서를 반복한다. -> Recursive  
1. 노드 v를 받아 dfs를 실행한다. dfs(v)
2. v에 방문했다는 것을 Visited에 표시한다.  
3. 노드 v를 출력한다.  
4. 노드 v에 인접하며, 방문하지 않은 노드를 찾아 dfs를 실행한다.
<img width=750 src="Graph/DFS.gif">


### Breadth First Search - BFS 
너비 우선 탐색이라고도 부른다.  
탐색을 시작하는 v와 가장 가까운 v를 먼저 탐색하며 멀리있는 v는 가장 나중에 탐색한다.  
Levelorder와 유사하다.  

1. 노드 v를 받아 bfs를 실행한다.
2. v에 방문했다는 것을 Visited에 표시한다. 
3. v를 출력한다.
4. v를 Queue에 추가한다.  
아래의 과정을 반복한다.
5. Queue의 Front에서 노드를 삭제하여 v에 할당한다.
6. v를 방문한 적이 있다면 인접한 노드로 이동한다.
7. v를 방문한 적이 없다면 Visited에 표시한다.
8. v를 출력한다.
9.  v를 Queue에 추가한다.

<img width=750 src="Graph/BFS.gif">
